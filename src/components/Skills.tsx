import { Cloud, Database, GitBranch, Globe, Smartphone, Zap } from "lucide-react";
              ...
              <CardContent className="p-6 text-center">
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  {category.skills.map((skill, skillIndex) => (
                    <span 
                      key={skillIndex}
                      className="px-3 py-1 text-sm bg-primary/10 text-primary rounded-full border border-primary/20 flex items-center gap-1"
                    >                    {skillLogos[skill] ? (
                      <img 
                        src={skillLogos[skill].src} 
                        alt={skillLogos[skill].alt}
                        className={`w-3.5 h-3.5 ${skillLogos[skill].invertInDarkMode ? 'dark:invert dark:brightness-200' : ''}`}
                        onError={(e) => { 
                ...
                <div className="flex justify-between items-center mb-2">
                  <div className="flex items-center gap-2">
                    {skillLogos[skill.name] ? (
                      <img 
                        src={skillLogos[skill.name].src} 
                        alt={skillLogos[skill.name].alt}
                        className={`w-5 h-5 ${skillLogos[skill.name].invertInDarkMode ? 'dark:invert dark:brightness-200' : ''}`}
                        onError={(e) => {
                          e.currentTarget.style.display = 'none';
                        }}
                      />
